...
module.exports = {
	pool: function (config) {
		var javaCon = jvm.import('nodejt400.JT400').createPoolSync(JSON.stringify(_.defaults(config || {}, defaultConfig)));
	...
	connect: function (config) {
		...
		return Q.nfcall(createConnection, JSON.stringify(_.defaults(config || {}, defaultConfig))).then(function (javaCon) {
			return createInstance(createConFrom(javaCon), insertListInOneStatment, false);
		});
	},
	useInMemoryDb: function () {
		var javaCon = jvm.newInstanceSync('nodejt400.HsqlClient');
		return createInstance(createConFrom(javaCon), standardInsertList, true);
	}
};
