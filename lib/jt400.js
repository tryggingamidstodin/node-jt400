...
function createConFrom(con) {
	return {
		...
		getMetaData: con.getMetaData.bind(con),
...
function createInstance(getConn, insertListFun) {
	var jt400 = {
		...
		getMetaData: function (opt) {
			return Q.nfcall(getConn().getMetaData, opt.catalog, opt.schema, opt.table || '%').then(JSON.parse);
		},
		getMetaDataAsStream: function (opt) {
			return new JdbcStream({
				jdbcStream: getConn().connection.getMetaDataAsStreamSync(opt.catalog, opt.schema, opt.table || '%')
			});
		},
