...
function createInstance(connection, insertListFun, inMemory) {
  ...
  const jt400 = mixinConnection({
    ...
    executeCmd(opt: CmdExecuteOptions) {
      const cmd = connection.connection.cmdSync(opt.cmdString)
      const cmdFunc = cmd.run.bind(cmd)

      return Q.nfcall(cmdFunc, opt.cmdString).then(JSON.parse)
    },
...
export interface CmdExecuteOptions {
  cmdString: string
}
...
export interface Connection extends BaseConnection {
  ...
  executeCmd: (options: CmdExecuteOptions) => Promise<any>
